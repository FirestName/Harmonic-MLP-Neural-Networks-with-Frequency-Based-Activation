...
def find_promising_lrs(lrs, losses, num_points=5):
    """Find promising learning rates based on the loss curve."""
    # Calculate rate of change of loss
    loss_diff = np.array([-1*(losses[i+1] - losses[i])/(np.log10(lrs[i+1]) - np.log10(lrs[i])) 
                         for i in range(len(losses)-1)])
    
    # Find the region of steepest descent
    smooth_diff = np.convolve(loss_diff, np.ones(5)/5, mode='valid')
    steepest_idx = np.argmax(smooth_diff) + 2  # +2 due to smoothing window
    
    # Get the learning rate at this point
    steepest_lr = lrs[steepest_idx]
    
    # Create a range around this point
    # Go one order of magnitude below and above
    min_lr = steepest_lr * 0.1
    max_lr = steepest_lr * 10
    
    # Create logarithmically spaced learning rates
    suggested_lrs = np.logspace(np.log10(min_lr), np.log10(max_lr), num_points)
    return list(suggested_lrs)
...
def main():
    ...
    # Find promising learning rates based on the loss curve
    learning_rates = find_promising_lrs(lrs, losses)
    print("\nTesting the following learning rates based on LR finder results:")
    print([f"{lr:.2e}" for lr in learning_rates])
